...
class CBValidateReceiptResource: CBAPIResource {
    ...
    init(receipt: CBReceipt ) {
        ...
        self.requestBody = PayloadBody(receipt: receipt.token, productId: receipt.productID, name: receipt.name,price: receipt.price, currencyCode: receipt.currencyCode,
                                       customerId: receipt.customerId ??  "", period: "\(receipt.period)" , periodUnit: "\(receipt.periodUnit)",first_name: receipt.first_name ?? "" ,last_name: receipt.last_name ?? "", email: receipt.email ?? "")
    }
